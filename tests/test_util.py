...
def test_intraclass_correlation():
    """Check `intraclass_correlation` works as intended."""

    # Each group has a variance of 0
    assert util.intraclass_correlation(['a', 'a', 'b', 'b'], [1, 1, 2, 2]) == 0.25 / (0.25 + 0)
    # The first group has a variance of 0.25 and the second one 0
    assert util.intraclass_correlation(['a', 'a', 'b', 'b'], [1, 2, 1, 1]) == 0.0625 / (0.0625 + 0.125)
    # Each group has a variance of 0.25
    assert util.intraclass_correlation(['a', 'a', 'b', 'b'], [1, 2, 1, 2]) == 0 / (0 + 0.25)
    # Each group has a variance of 0.25 and the groups are unbalanced
    assert util.intraclass_correlation(
        ['a', 'a', 'a', 'a', 'b', 'b'],
        [1, 1, 2, 2, 1, 1]
